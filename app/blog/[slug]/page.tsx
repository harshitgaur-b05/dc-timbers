...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  return {
    ...
    description: post.metaDescription || post.excerpt || `Read ${post.title} on the DCtimbers trade journal.`,
  };
}
...
export default async function BlogPostDetailPage({ params }: Params) {
  ...
  const rawPost = await BlogPost.findOne({ slug, published: true }).lean();
  if (!rawPost) notFound();
  ...
  return (
    <article className="py-12 px-8 max-w-4xl mx-auto w-full">
      ...
      <header className="mb-8 pb-8 border-b" style={{ borderColor: "rgba(128,117,109,0.2)" }}>
        <span
          className="text-xs uppercase tracking-widest font-black"
          style={{ fontFamily: "var(--font-mono)", color: "var(--color-secondary)" }}
        >
          {post.publishedAt ? new Date(post.publishedAt).toLocaleDateString("en-GB") : "Published"}
        ...
        {post.excerpt && (
          <p className="text-xl leading-relaxed opacity-80" style={{ color: "var(--color-on-surface-variant)" }}>
            {post.excerpt}
          </p>
        )}
      </header>
      ...
      <footer className="mt-12 pt-8 border-t flex justify-between items-center" style={{ borderColor: "rgba(128,117,109,0.2)" }}>
        <Link
          href="/blog"
